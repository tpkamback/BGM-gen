...
@pytest.mark.parametrize(
    ...
    [
        (
            "./test/test_modify_video",
            {
                "test_gen_music": [
                    "./test/test_modify_video/[suno]test_gen_music(1).mp3",
                    "./test/test_modify_video/[suno]test_gen_music.mp3",
                ]
            },
        ),
    ],
...
@pytest.mark.parametrize(
    ...
    [
        (
            "./test/test_modify_video",
            [
                "./test/test_modify_video/test_thumbnail_0.png",
                "./test/test_modify_video/test_thumbnail_1.jpg",
            ],
        ),
    ],
)
...
class TestAPIRequest(unittest.TestCase):
    ...
    def test_api_request_success(self):
        ...
        expected_hases = {
            f: calculate_file_hash(os.path.join(expected_dir, f))
            for f in os.listdir(expected_dir)
        }
        output_hashes = {
            f: calculate_file_hash(os.path.join(self.output_dir, f))
